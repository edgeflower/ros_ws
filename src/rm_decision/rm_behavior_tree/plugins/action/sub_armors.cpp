...
/*
  ...
  构造函数使用初始化列表调用基类构造函数，
  这里基类是 BT::RosTopicSubNode<auto_aim_interfaces::msg::Armors>，
  该基类封装了基于 ROS2 话题订阅的行为树节点，
  模板参数 auto_aim_interfaces::msg::Armors 表示该节点订阅的数据消息类型。
*/
SubArmorsAction::SubArmorsAction(
    const std::string & name, 
    const BT::NodeConfig & conf, 
    const BT::RosNodeParams & params)
: BT::RosTopicSubNode<armor_interfaces::msg::Armors>(name, conf, params)
{
...
/*
  ...
  参数：
    - last_msg：一个指向 auto_aim_interfaces::msg::Armors 消息的共享指针，
...
BT::NodeStatus SubArmorsAction::onTick(
    const std::shared_ptr<armor_interfaces::msg::Armors> & last_msg)
{
    if(last_msg){
