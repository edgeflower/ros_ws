...
int main(int argc, char ** argv)
{
  ...
  nh->declare_parameter<double>("nearDecayTime", nearDecayTime);
  ...
  nh->get_parameter("nearDecayTime", nearDecayTime);
  ...
  std::string odometry_topic = nh->declare_parameter<std::string>("odometry_topic", "odometry");
  std::string laser_cloud_topic = nh->declare_parameter<std::string>("laser_cloud_topic", "registered_scan");
      ...
      for (int ind = 0; ind < kTerrainVoxelNum; ind++) {
        if (
          ...
          for (int i = 0; i < laserCloudDwzSize; i++) {
            ...
            // 根据距离选择不同的衰减时间
            double effectiveDecayTime = (dis < noDecayDis) ? nearDecayTime : decayTime;
            if (
              point.z - vehicleZ > minRelZ - disRatioZ * dis &&
              point.z - vehicleZ < maxRelZ + disRatioZ * dis &&
              laserCloudTime - systemInitTime - point.intensity < effectiveDecayTime &&
