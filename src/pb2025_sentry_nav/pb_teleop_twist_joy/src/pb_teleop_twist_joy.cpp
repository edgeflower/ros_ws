...
: Node("teleop_twist_joy_node", options)
{
  ...
  this->declare_parameter<std::string>("cmd_vel_name","cmd_vel");
  ...
  this->get_parameter("cmd_vel_name",cmd_vel_name_);
  ...
  if (publish_stamped_twist_) {
    cmd_vel_stamped_pub_ = this->create_publisher<geometry_msgs::msg::TwistStamped>(cmd_vel_name_, 10);
  } else {
    cmd_vel_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(cmd_vel_name_, 10);
  }
  joint_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>("cmd_gimbal_big_joint", 10);
