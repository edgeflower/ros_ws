...
class RosNodeControlInterface {
    public:
        ...
        virtual void setSlamCloudBodyRect(int front_mm, int back_mm, int left_mm, int right_mm) = 0;
        virtual int slamCloudBodyFront() const = 0;
        virtual int slamCloudBodyBack() const = 0;
        virtual int slamCloudBodyLeft() const = 0;
        virtual int slamCloudBodyRight() const = 0;
        virtual bool slamCloudBodyFilterEnabled() const = 0;
    };
...
void publishRgb(capture_Image_List_t *stream) {
    ...
    void publishPC2XYZRGBA(capture_Image_List_t* stream, int idx) 
    {
        ...
        // Body rect filter: read config and latest odom pose
        const bool body_filter = getRosNodeControl()->slamCloudBodyFilterEnabled();
        float odom_x = 0.0f, odom_y = 0.0f, odom_yaw = 0.0f;
        float front_m = 0.0f, back_m = 0.0f, left_m = 0.0f, right_m = 0.0f;
        if (body_filter) {
            std::lock_guard<std::mutex> lock(odom_pose_mutex_);
            odom_x = latest_odom_x_;
            odom_y = latest_odom_y_;
            odom_yaw = latest_odom_yaw_;
            front_m  = static_cast<float>(getRosNodeControl()->slamCloudBodyFront())  / 1000.0f;
            back_m   = static_cast<float>(getRosNodeControl()->slamCloudBodyBack())   / 1000.0f;
            left_m   = static_cast<float>(getRosNodeControl()->slamCloudBodyLeft())   / 1000.0f;
            right_m  = static_cast<float>(getRosNodeControl()->slamCloudBodyRight())  / 1000.0f;
        }

        const float cos_yaw = body_filter ? std::cos(-odom_yaw) : 0.0f;
        const float sin_yaw = body_filter ? std::sin(-odom_yaw) : 0.0f;
        ...
        for(uint32_t i = 0; i < points; i++) {
            ...
            float px, py, pz;
#ifdef ROS2
                px = static_cast<float>(ptr[0]) / 10000.0f;
                py = static_cast<float>(ptr[1]) / 10000.0f;
                pz = static_cast<float>(ptr[2]) / 10000.0f;
#else
                px = (1.0f * ptr[0]) / 1e4f;
                py = (1.0f * ptr[1]) / 1e4f;
                pz = (1.0f * ptr[2]) / 1e4f;
            ...
            // Filter points within body rectangle in vehicle-local frame
            if (body_filter) {
                float dx = px - odom_x;
                float dy = py - odom_y;
                float local_x = dx * cos_yaw - dy * sin_yaw;
                float local_y = dx * sin_yaw + dy * cos_yaw;
                if (local_x >= -back_m && local_x <= front_m &&
                    local_y >= -right_m && local_y <= left_m) continue;
            }
            ...
            *iter_intensity = 0.0f; ++iter_intensity;
            ++kept;
        }

        // Trim message data to actual kept points
        msg.width = kept;
        msg.data.resize(kept * msg.point_step);
    ...
    void publishOdometry(capture_Image_List_t* stream, OdometryType odom_type, bool show_path, bool show_camerapose) {
            ...
            if (data_len == sizeof(ros_odom_convert_complete_t)) {
                ...
                {
                    std::lock_guard<std::mutex> lock(odom_pose_mutex_);
                    latest_odom_x_ = static_cast<float>(msg.pose.pose.position.x);
                    latest_odom_y_ = static_cast<float>(msg.pose.pose.position.y);
                    latest_odom_z_ = static_cast<float>(msg.pose.pose.position.z);
                    // Extract yaw from quaternion: yaw = atan2(2(wz+xy), 1-2(y²+z²))
                    float qx = static_cast<float>(odom_data->orient[0]) / 1e6f;
                    float qy = static_cast<float>(odom_data->orient[1]) / 1e6f;
                    float qz = static_cast<float>(odom_data->orient[2]) / 1e6f;
                    float qw = static_cast<float>(odom_data->orient[3]) / 1e6f;
                    latest_odom_yaw_ = std::atan2(2.0f * (qw * qz + qx * qy), 1.0f - 2.0f * (qy * qy + qz * qz));
                }
...
private:
    ...
    void initialize_publishers() {
        ...
        float latest_odom_x_ = 0.0f;
        float latest_odom_y_ = 0.0f;
        float latest_odom_z_ = 0.0f;
